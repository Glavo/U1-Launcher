...
public class RemoteVersion {
    ...
    public static RemoteVersion fetch(UpdateChannel channel, String url) throws IOException {
        try {
            ...
            String force = Optional.ofNullable(response.get("force")).map(JsonElement::getAsString).orElse(null);
            if (Pack200Utils.isSupported() && packXZUrl != null && packXZHash != null) {
                return new RemoteVersion(channel, version, packXZUrl, Type.PACK_XZ, new IntegrityCheck("SHA-1", packXZHash), "true".equalsIgnoreCase(force));
            } else if (jarUrl != null && jarHash != null) {
                return new RemoteVersion(channel, version, jarUrl, Type.JAR, new IntegrityCheck("SHA-1", jarHash), "true".equalsIgnoreCase(force));
            } else {
